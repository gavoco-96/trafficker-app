// Trafficker Pro v2.5 — modularizado (Fase 4)
import { useState, useEffect, useCallback, useRef, Component } from "react";

// ─── MÓDULOS PROPIOS ──────────────────────────────────────────────────────────
import { css } from "./lib/styles.js";
import { SUPA_URL, SUPA_KEY, H, HL, db } from "./lib/db.js";
import {
...
import {
  ...
  localDateStr, localDateTimeStr, filterByPeriod,
} from "./lib/utils.js";
import {
  LEGAL_TITULAR, LEGAL_MARCA, LEGAL_EMPRESA, LEGAL_CONTACTO,
  LEGAL_DOMINIO, LEGAL_VIGENCIA, LEGAL_ANIO,
  LegalModal, LegalContenido, LegalPagina, LegalFooter,
} from "./components/Legal.jsx";
import {
  FB_METRICAS_DISPONIBLES, PAISES_INFO, paisNombre, paisBandera,
  fetchMetricasPorNivel, diagnosticarSaludFB, fetchMetricasPorCampana,
  fetchGastoPorPais, fetchPresupuestoPorPais,
} from "./lib/api-facebook.js";
import {
  TT_API, TT_METRICAS, getCuentasTT, getCuentasTTActivas, getTokenTT, ttListo,
  ttFetch, diagnosticarSaludTT, ttLeads, fetchMetricasTTPorNivel,
  fetchGastoPorPaisTT, fetchPresupuestoTT,
} from "./lib/api-tiktok.js";
import {
  AUTOSYNC_HORA_CIERRE, AUTOSYNC_INTERVALO_MS, AUTOSYNC_REFRESCO_HOY_MIN,
  construirRegistroDesdeFB, autoSyncFechaCliente, ejecutarAutoSyncCliente,
  useAutoSyncDiario,
...
// HL viene de lib/db.js (headers de Supabase para tablas auxiliares)
...
function AdminPanel({ clients, onLogout, onUpdate, onAddClient, onDeleteClient, banners, onSaveBanners, globalConfig, onSaveGlobalConfig, filmmakers, saveFilmmakers }) {
  ...
  const Sidebar = () => (
    <div className="sidebar">
      ...
      <div className="sidebar-footer"><DbStatus /><button className="btn btn-ghost btn-sm btn-full" style={{ marginTop: 10 }} onClick={onLogout}>Cerrar sesión</button><LegalFooter /></div>
    </div>
  );
...
export default function App() {
  ...
  // Meta, Google y TikTok exigen que la política de privacidad y los términos
  // sean accesibles públicamente por URL. Estas rutas no requieren sesión:
