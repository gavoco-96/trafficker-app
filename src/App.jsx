...
const ANTECEDENTES_CAMPOS = [
  { key: "inversion", label: "Inversión", prefix: "$", tipo: "costo", desc: "Entre más bajo mejor (objetivo: reducir)" },
  { key: "ticket_promedio", label: "Ticket Promedio", prefix: "$", tipo: "neutro", desc: "Valor promedio por venta" },
  { key: "ganancia_bruta", label: "Ganancia Bruta", prefix: "$", tipo: "beneficio", desc: "Entre más alto mejor" },
  { key: "ganancia_neta", label: "Ganancia Neta", prefix: "$", tipo: "beneficio", desc: "Entre más alto mejor" },
  { key: "roas_marketing", label: "ROAS Marketing", prefix: "", suffix: "x", tipo: "beneficio", desc: "Entre más alto mejor" },
  { key: "roas_empresarial", label: "ROAS Empresarial", prefix: "", suffix: "x", tipo: "beneficio", desc: "Entre más alto mejor" },
  { key: "cpa", label: "Costo x Resultado (CPA)", prefix: "$", tipo: "costo", desc: "Objetivo: no exceder +20% del histórico" },
  { key: "cpr", label: "Costo x Venta (CPR)", prefix: "$", tipo: "costo", desc: "Objetivo: no exceder +20% del histórico" },
  { key: "cpc", label: "Costo x Clic (CPC)", prefix: "$", tipo: "costo", desc: "Objetivo: no exceder +20% del histórico" },
  { key: "impresiones", label: "Impresiones", prefix: "", tipo: "neutro", desc: "Alcance de los anuncios" },
  { key: "alcance", label: "Alcance", prefix: "", tipo: "neutro", desc: "Personas únicas alcanzadas" },
  { key: "clics_enlace", label: "Clics en el Enlace", prefix: "", tipo: "beneficio", desc: "Entre más alto mejor" },
  { key: "resultados", label: "Resultados", prefix: "", tipo: "beneficio", desc: "Total de resultados obtenidos" },
  { key: "ventas", label: "Ventas", prefix: "", tipo: "beneficio", desc: "Total de ventas cerradas" },
];
...
const css = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .input-prefix span{position:absolute;left:12px;color:var(--muted);font-size:14px;pointer-events:none}
...
// ─── UTILS ────────────────────────────────────────────────────────────────────
const fmt = (n, dec = 0) => isNaN(n) || n === "" || n === null || n === undefined ? "—" : Number(n).toLocaleString("es-EC", { minimumFractionDigits: dec, maximumFractionDigits: dec });
const fmtVal = (n, dec = 2) => (n === "" || n === null || n === undefined || isNaN(Number(n))) ? "" : Number(n).toFixed(dec);
...
function exportAntecedentes(antecedentes, clientName, formato) {
  if (!antecedentes || antecedentes.length === 0) return;
  const headers = ["Red", "Período", ...ANTECEDENTES_CAMPOS.map(c => c.label)];
  const rows = antecedentes.map(a => [
    a.red || "", `${a.fechaInicio || ""} - ${a.fechaFin || ""}`,
    ...ANTECEDENTES_CAMPOS.map(c => a[c.key] || "")
  ]);
  const allRows = [headers, ...rows];
  if (formato === "csv") {
    const csv = allRows.map(r => r.map(v => `"${v}"`).join(",")).join("\n");
    downloadFile(`antecedentes_${clientName}.csv`, csv, "text/csv");
  } else {
    // XLS (tab-separated)
    const xls = allRows.map(r => r.join("\t")).join("\n");
    downloadFile(`antecedentes_${clientName}.xls`, xls, "application/vnd.ms-excel");
  }
}
          ...
          return (
            <div key={svc.id} className="stage-block">
              <div className="stage-tooltip">
                <div style={{ fontWeight: 600, marginBottom: 4 }}>{svc.nombre} {!activo && <span style={{ color: "var(--orange)", fontSize: 10 }}>UPGRADE</span>}</div>
...
function ChecklistPanel({ client, onUpdate }) {
  const checklist = client.checklist || {};
  const servicios = client.serviciosContratados || [];
  function toggle(svcId, sub) {
    onUpdate({ ...client, checklist: { ...checklist, [svcId]: { ...(checklist[svcId] || {}), [sub]: !checklist[svcId]?.[sub] } } });
  }
  return (
    <div>
      {SERVICIOS_DEFAULT.map(svc => {
...
function CuentasPanel({ client, onUpdate, readOnly }) {
  ...
  function addCuenta() { onUpdate({ ...client, cuentas: [...cuentas, { red: "facebook", usuario: "", password: "" }] }); }
  function upd(i, f, v) { onUpdate({ ...client, cuentas: cuentas.map((c, xi) => xi === i ? { ...c, [f]: v } : c) }); }
  function rem(i) { onUpdate({ ...client, cuentas: cuentas.filter((_, xi) => xi !== i) }); }
  if (readOnly) return (
  ...
  return (
    <div className="card">
      <div className="sec-header"><div className="card-title" style={{ margin: 0 }}>Cuentas y accesos</div><button className="btn btn-ghost btn-sm" onClick={addCuenta}>+ Añadir cuenta</button></div>
...
function ContratosPanel({ client, onUpdate }) {
  ...
  function addContrato() {
    ...
    const svcs = (client.serviciosContratados || []).map(id => {
      const s = SERVICIOS_DEFAULT.find(x => x.id === id);
      return { id, nombre: s?.nombre || id, precio: "" };
    });
    const nuevo = { id: "ct" + Date.now(), fechaInicio: hoy, fechaFin: "", servicios: svcs, cuotas: [{ monto: "", pagado: false, fecha: "" }, { monto: "", pagado: false, fecha: "" }], notas: "", guardado: false };
    onUpdate({ ...client, contratos: [...contratos, nuevo] });
  }

  function saveContrato(i, ct) {
    onUpdate({ ...client, contratos: contratos.map((c, xi) => xi === i ? { ...ct, guardado: true } : c) });
  }

  function removeContrato(i) {
    if (!window.confirm("¿Eliminar este contrato?")) return;
    onUpdate({ ...client, contratos: contratos.filter((_, xi) => xi !== i) });
  }
  ...
  return (
    <div>
      ...
      {contratos.map((ct, i) => <ContratoItem key={ct.id} ct={ct} i={i} client={client} onSave={ct2 => saveContrato(i, ct2)} onRemove={() => removeContrato(i)} />)}
    </div>
  );
}

function ContratoItem({ ct, i, client, onSave, onRemove }) {
  const [local, setLocal] = useState({ ...ct });
  const upd = (k, v) => setLocal(p => ({ ...p, [k]: v }));

  // Auto-sum total from servicios
  useEffect(() => {
  ...
  // Auto-distribute cuotas equally
  useEffect(() => {
    const total = parseFloat(local.totalContrato) || 0;
    if (total > 0 && local.cuotas?.length > 0) {
      const porCuota = (total / local.cuotas.length).toFixed(2);
      setLocal(p => ({ ...p, cuotas: p.cuotas.map(c => ({ ...c, monto: porCuota })) }));
    }
  }, [local.totalContrato, local.cuotas?.length]);

  function updSvc(si, v) {
    const svcs = local.servicios.map((s, xi) => xi === si ? { ...s, precio: v } : s);
    setLocal(p => ({ ...p, servicios: svcs }));
  }

  function updCuota(ci, f, v) {
    setLocal(p => ({ ...p, cuotas: p.cuotas.map((c, xi) => xi === ci ? { ...c, [f]: v } : c) }));
  }

  function addCuota() {
    setLocal(p => ({ ...p, cuotas: [...(p.cuotas || []), { monto: "", pagado: false, fecha: "" }] }));
  }

  function removeCuota(ci) {
    if (local.cuotas.length <= 1) return;
    setLocal(p => ({ ...p, cuotas: p.cuotas.filter((_, xi) => xi !== ci) }));
  }

  const pagadas = (local.cuotas || []).filter(c => c.pagado).length;
  const total = local.cuotas?.length || 0;
  const estado = pagadas === total ? { label: "Pago completo", cls: "badge-paid" } : pagadas > 0 ? { label: `${pagadas}/${total} cuotas pagadas`, cls: "badge-partial" } : { label: "Pendiente", cls: "badge-pending" };
    ...
    <div className="contrato-card">
      ...
      <div className="field" style={{ marginBottom: "1rem" }}>
        ...
        {(local.servicios || []).map((s, si) => (
          <div key={si} style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
            <span style={{ fontSize: 13, minWidth: 120, color: "var(--text)" }}>{s.nombre}</span>
            <div className="input-prefix" style={{ flex: 1 }}>
              <span>$</span>
              <input type="number" value={s.precio} onChange={e => updSvc(si, e.target.value)} placeholder="0.00" />
            </div>
          </div>
        ))}
      </div>
      <div className="field" style={{ marginBottom: "1rem" }}>
        ...
        <div className="input-prefix">
          <span>$</span>
          <input type="number" value={local.totalContrato} onChange={e => upd("totalContrato", e.target.value)} placeholder="0.00" />
        </div>
        <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 4 }}>Se calcula automáticamente sumando los servicios</div>
      </div>
      <div style={{ marginBottom: "1rem" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
          <label style={{ fontSize: 12, fontWeight: 500, color: "var(--muted)", textTransform: "uppercase", letterSpacing: ".06em" }}>Cuotas de pago</label>
          <button className="btn btn-ghost btn-sm" onClick={addCuota}>+ Añadir cuota</button>
        </div>
        <div className="grid2">
          {(local.cuotas || []).map((c, ci) => (
            <div key={ci} style={{ background: "var(--bg)", borderRadius: 8, padding: 12, position: "relative" }}>
              {local.cuotas.length > 1 && <button style={{ position: "absolute", top: 8, right: 8, background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 16 }} onClick={() => removeCuota(ci)}>×</button>}
              <div style={{ fontWeight: 600, fontSize: 12, marginBottom: 8, color: "var(--muted)", textTransform: "uppercase", letterSpacing: ".06em" }}>
                Cuota {ci + 1} ({total > 0 ? Math.round(100 / total) : 0}%)
              </div>
              <div className="input-prefix" style={{ marginBottom: 8 }}>
                <span>$</span>
                <input type="number" value={c.monto} onChange={e => updCuota(ci, "monto", e.target.value)} placeholder="0.00" />
              </div>
              <input type="date" value={c.fecha} onChange={e => updCuota(ci, "fecha", e.target.value)} style={{ marginBottom: 8 }} />
              <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, cursor: "pointer" }}>
                <input type="checkbox" checked={!!c.pagado} onChange={e => updCuota(ci, "pagado", e.target.checked)} style={{ width: 16, height: 16 }} />
                Pagado
              </label>
            </div>
          ))}
      ...
      <div className="field" style={{ marginBottom: "1rem" }}>
        ...
        <textarea value={local.notas} onChange={e => upd("notas", e.target.value)} placeholder="Observaciones, acuerdos especiales..." />
      </div>
      <button className="btn btn-green btn-sm" onClick={() => onSave(local)}>
...
// ─── ANTECEDENTES ─────────────────────────────────────────────────────────────
function getColor(campo, valor, historico) {
  ...
  const campoInfo = ANTECEDENTES_CAMPOS.find(c => c.key === campo);
  if (!campoInfo) return "ant-cell-neutral";
  if (campoInfo.tipo === "beneficio") {
    if (v >= maxVal * 0.9) return "ant-cell-green";
    if (v <= minVal * 1.1) return "ant-cell-red";
    return "ant-cell-amber";
  }
  if (campoInfo.tipo === "costo") {
    const refMin = Math.min(...vals);
    if (v <= refMin * 1.05) return "ant-cell-green";
    if (v > refMin * 1.2) return "ant-cell-red";
    return "ant-cell-amber";
  }
...
function AntecedentesPanel({ client, onUpdate, readOnly }) {
  ...
  function saveAnt() {
    ...
    const nuevo = { ...form, id: "ant" + Date.now() };
    onUpdate({ ...client, antecedentes: [...ants, nuevo] });
    setForm({ red: REDES_PAUTA[0], fechaInicio: "", fechaFin: "" });
    setShowForm(false);
  }

  function removeAnt(id) {
    if (!window.confirm("¿Eliminar este antecedente?")) return;
    onUpdate({ ...client, antecedentes: ants.filter(a => a.id !== id) });
  }
  ...
  return (
    <div>
      <div className="sec-header">
        <div>
          <div className="sec-title">Antecedentes históricos</div>
          <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 2 }}>Historial de pauta por red y período</div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          {ants.length > 0 && <>
            <button className="btn btn-ghost btn-sm" onClick={() => exportAntecedentes(ants, client.name, "csv")}>⬇ CSV</button>
            <button className="btn btn-ghost btn-sm" onClick={() => exportAntecedentes(ants, client.name, "xls")}>⬇ XLS</button>
          </>}
          {!readOnly && <button className="btn btn-primary btn-sm" onClick={() => setShowForm(s => !s)}>{showForm ? "Cancelar" : "+ Nuevo registro"}</button>}
        </div>
      </div>

      {/* Leyenda de colores */}
      <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
        {[["ant-cell-green", "Óptimo"], ["ant-cell-amber", "Aceptable"], ["ant-cell-red", "Atención"]].map(([cls, label]) => (
          <div key={cls} style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <span className={cls} style={{ fontSize: 11 }}>Ej</span>
            <span style={{ fontSize: 11, color: "var(--muted)" }}>{label}</span>
          </div>
        ))}
        <span style={{ fontSize: 11, color: "var(--muted)" }}>• Rojo = CPA/CPC/CPR supera +20% del mínimo histórico</span>
      </div>
      ...
      {showForm && !readOnly && (
        <div className="card" style={{ borderColor: "rgba(124,58,237,.4)" }}>
          ...
          <div className="form-row">
            <div className="field"><label>Red de pauta</label>
              <select value={form.red} onChange={e => f("red", e.target.value)}>
                {REDES_PAUTA.map(r => <option key={r} value={r}>{r}</option>)}
              </select>
            </div>
            <div></div>
          </div>
          <div className="form-row">
            <div className="field"><label>Fecha inicio del período</label><input type="date" value={form.fechaInicio} onChange={e => f("fechaInicio", e.target.value)} /></div>
            <div className="field"><label>Fecha fin del período</label><input type="date" value={form.fechaFin} onChange={e => f("fechaFin", e.target.value)} /></div>
          </div>
          <div className="ant-form-grid">
            {ANTECEDENTES_CAMPOS.map(campo => (
              <div key={campo.key} className="field">
                ...
                <div className="input-prefix">
                  {campo.prefix && <span>{campo.prefix}</span>}
                  <input type="number" value={form[campo.key] || ""} onChange={e => f(campo.key, e.target.value)} placeholder="0" style={campo.prefix ? {} : {}} />
                </div>
              </div>
            ))}
          </div>
          <button className="btn btn-primary btn-sm" onClick={saveAnt}>Guardar antecedente</button>
        </div>
      )}

      {ants.length === 0 && !showForm && <div className="empty"><div style={{ fontSize: 28, marginBottom: 8, opacity: .3 }}>📊</div><div>Sin antecedentes históricos.</div></div>}
          ...
          <table className="tbl">
            <thead>
              <tr>
                <th>Red</th>
                <th>Período</th>
                {ANTECEDENTES_CAMPOS.map(c => <th key={c.key}>{c.label}</th>)}
                {!readOnly && <th></th>}
              </tr>
            </thead>
            <tbody>
              {ants.map(a => (
                <tr key={a.id}>
                  ...
                  {ANTECEDENTES_CAMPOS.map(c => {
                    const val = a[c.key];
                    const cls = getColor(c.key, val, ants);
                    const display = val !== "" && val !== undefined && val !== null ? `${c.prefix || ""}${fmt(Number(val), 2)}${c.suffix || ""}` : "—";
                    return <td key={c.key}><span className={cls}>{display}</span></td>;
                  })}
                  {!readOnly && <td><button className="btn btn-danger btn-sm" onClick={() => removeAnt(a.id)}>×</button></td>}
                </tr>
              ))}
            </tbody>
          </table>
...
function ClientForm({ initial, onSave, onCancel }) {
  const blank = { name: "", username: "", password: "", niche: "whatsapp", color: "#7C3AED", logo: "", producto: "", email: "", telefono: "", direccion: "", representante: "", serviciosContratados: [], checklist: {}, cuentas: [], contratos: [], antecedentes: [] };
  ...
  function addCustomSvc() {
    ...
    const subs = nuevoSvc.subetapas.split(",").map(s => s.trim()).filter(Boolean);
    setCustomSvcs(p => [...p, { id, nombre: nuevoSvc.nombre, subetapas: subs }]);
  ...
  return (
    <div style={{ maxWidth: 680 }}>
      ...
      <div className="form-row">
        <div className="field"><label>Representante</label><input type="text" value={form.representante} onChange={e => f("representante", e.target.value)} placeholder="Nombre completo" /></div>
      ...
      <div className="chips-wrap">
        {todos.map(svc => <div key={svc.id} className={`servicio-chip ${form.serviciosContratados.includes(svc.id) ? "selected" : ""}`} onClick={() => toggleSvc(svc.id)}>{form.serviciosContratados.includes(svc.id) ? "✓ " : ""}{svc.nombre}</div>)}
      </div>
      <div style={{ marginTop: 12, background: "var(--surface2)", borderRadius: 10, padding: 12 }}>
        ...
        <div className="form-row" style={{ marginBottom: 8 }}>
          <input type="text" value={nuevoSvc.nombre} onChange={e => setNuevoSvc(p => ({ ...p, nombre: e.target.value }))} placeholder="Nombre del servicio" />
          <input type="text" value={nuevoSvc.subetapas} onChange={e => setNuevoSvc(p => ({ ...p, subetapas: e.target.value }))} placeholder="Subetapas separadas por coma" />
        </div>
        <button className="btn btn-ghost btn-sm" onClick={addCustomSvc}>Añadir servicio</button>
      </div>
      <div style={{ display: "flex", gap: 10, marginTop: "1.5rem" }}>
        <button className="btn btn-primary" onClick={() => { if (!form.name || !form.username || !form.password) return alert("Completa nombre, usuario y contraseña."); onSave(form); }}>
...
function AddRecordForm({ client, onSave, onCancel }) {
  ...
  return (
    <div style={{ maxWidth: 560 }}>
      ...
      {isLaunch && <><div className="section-label">Lanzamiento / Leads</div><div className="form-row"><Num lbl="Clientes potenciales" fk="clientesPotenciales" /><Num lbl="Formularios" fk="formularios" /></div></>}
  ...
  return (
    <div className="filter-bar">
      <div className="period-pills">
        {["7d", "mtd", "30d", "all", "custom"].map(p => <button key={p} className={`pill ${period === p ? "active" : ""}`} onClick={() => setPeriod(p)}>{p === "7d" ? "7 días" : p === "mtd" ? "Este mes" : p === "30d" ? "30 días" : p === "all" ? "Todo" : "Rango"}</button>)}
      </div>
      ...
      <div className="content">
        ...
        {tab === "info" && <div>
          ...
          <div className="card">
            ...
            <div className="grid2">
              <div>{[["Negocio", client.name], ["Representante", client.representante], ["Producto/Servicio", client.producto], ["Teléfono", client.telefono], ["Email", client.email], ["Dirección", client.direccion]].map(([l, v]) => <div key={l} className="info-row"><span className="info-label">{l}</span><span>{v || <span style={{ color: "var(--muted)" }}>—</span>}</span></div>)}</div>
              <div>{[["Usuario", client.username], ["Nicho", nicheLabel], ["Servicios", (client.serviciosContratados || []).map(id => SERVICIOS_DEFAULT.find(s => s.id === id)?.nombre || id).join(", ") || "—"]].map(([l, v]) => <div key={l} className="info-row"><span className="info-label">{l}</span><span>{v}</span></div>)}</div>
            </div>
          </div>
        </div>}
        ...
        {tab === "metricas" && <>
          ...
          <div className="grid4" style={{ marginBottom: "1rem" }}>
            <MetricCard label="Inversión" value={"$" + t.inversion} /><MetricCard label="Alcance" value={t.alcance} /><MetricCard label="CPM" value={"$" + t.cpm} /><MetricCard label="ROAS" value={(t.roas || "—") + "x"} highlight />
          </div>
  ...
  return (
    <div className="app">
      <div className="sidebar">
        <div className="sidebar-logo"><div className="sidebar-logo-badge">Mi panel</div><div className="sidebar-logo-name">{client.name}</div><div className="sidebar-logo-role">Cliente</div></div>
        <div className="nav">
          <div className="nav-label">Vistas</div>
          {["resumen", "detalle", "antecedentes", "reporte"].map(v => <div key={v} className={`nav-item ${tab === v ? "active" : ""}`} onClick={() => setTab(v)}><div className="nav-dot" style={{ background: tab === v ? "var(--accent)" : "var(--border)" }} />{v === "resumen" ? "Resumen" : v === "detalle" ? "Detalle diario" : v === "antecedentes" ? "Histórico" : "Reporte IA"}</div>)}
        </div>
      ...
      <div className="main">
        <div className="topbar">
          <div className="topbar-title">{tab === "resumen" ? "Resumen" : tab === "detalle" ? "Detalle diario" : tab === "antecedentes" ? "Histórico de pauta" : "Reporte IA"}</div>
          <PeriodFilter period={period} setPeriod={setPeriod} from={from} setFrom={setFrom} to={to} setTo={setTo} />
        </div>
        <div className="content">
          <ProgressBar client={client} />
          ...
          {tab === "reporte" && <div>
            ...
            <button className="btn btn-primary" disabled={loadingReport || rows.length === 0} onClick={() => generateReport(client, rows, setReport, setLoadingReport)}>{loadingReport ? "Generando..." : "Generar reporte IA"}</button>
            {(report || loadingReport) && <div className="ai-report"><div className="ai-report-header"><span>✦</span> Reporte · {client.name}</div><div className={`ai-report-body ${loadingReport && !report ? "streaming-cursor" : ""}`}>{report || " "}{loadingReport && report && <span className="streaming-cursor" />}</div></div>}
          </div>}
        </div>
      </div>
...
function AdminPanel({ clients, onLogout, onUpdate, onAddClient, onDeleteClient }) {
  ...
  return (
    <div className="app"><Sidebar />
      <div className="main">
        <div className="topbar"><div className="topbar-title">{view === "clientes" ? "Mis clientes" : "Resumen general"}</div>{view === "clientes" && <button className="btn btn-primary btn-sm" onClick={() => setAddingClient(true)}>+ Nuevo cliente</button>}</div>
        <div className="content">
...
function LoginScreen({ onLogin }) {
  ...
  function attempt() { if (!onLogin(user.trim(), pass)) setErr("Usuario o contraseña no encontrados. Verifica tus datos."); }
  return (
    <div className="login">
...
export default function App() {
  ...
  const [clients, setClients] = useState(() => S.get("trafficker_clients_v4") || SEED_CLIENTS);

  // Migrar clientes viejos que no tengan antecedentes
  useEffect(() => {
    const migrated = clients.map(c => ({ antecedentes: [], ...c }));
    setClients(migrated);
  }, []);

  function saveClients(c) { setClients(c); S.set("trafficker_clients_v4", c); }
  ...
  function addClient(data) { saveClients([...clients, { antecedentes: [], ...data, id: "c" + Date.now(), records: [] }]); }
  ...
  return (
    <>
      ...
      {session?.role === "client" && !clientSession && <LoginScreen onLogin={handleLogin} />}
    </>
  );
}
